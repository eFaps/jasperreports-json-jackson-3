...
public class JsonDataSource extends JRAbstractTextDataSource implements JsonData<JsonDataSource>, RandomAccessDataSource {
    ...
    public void moveFirst() throws JRException {
        ...
        final JsonNode result = getJsonData(jsonTree, selectExpression);
    ...
    public Object getFieldValue(JRField jrField) throws JRException
    {
        ...
        final JsonNode selectedObject = getJsonData(currentJsonNode, expression);

        if(Object.class != valueClass)
        {
            final boolean hasValue = selectedObject != null
    ...
    protected JsonNode getJsonData(JsonNode rootNode, String jsonExpression) throws JRException {
        ...
        final StringTokenizer tokenizer = new StringTokenizer(jsonExpression, PROPERTY_SEPARATOR);

        while(tokenizer.hasMoreTokens()) {
            final String currentToken = tokenizer.nextToken();
            final int currentTokenLength = currentToken.length();
            final int indexOfLeftSquareBracket = currentToken.indexOf(ARRAY_LEFT);
            ...
            if (indexOfLeftSquareBracket != -1) {
                ...
                if (indexOfLeftSquareBracket > 0) {
                    ...
                    final String property = currentToken.substring(0, indexOfLeftSquareBracket);
                    tempNode = goDownPathWithAttribute(tempNode, property);
                }

                final String arrayOperators = currentToken.substring(indexOfLeftSquareBracket);
                final StringTokenizer arrayOpsTokenizer = new StringTokenizer(arrayOperators,ARRAY_RIGHT);
                while(arrayOpsTokenizer.hasMoreTokens()) {
                    if (tempNode == null || tempNode.isMissingNode() || !tempNode.isArray()) {
                        return null;
                    }

                    final String currentArrayOperator = arrayOpsTokenizer.nextToken();
    ...
    protected JsonNode goDownPathWithAttribute(JsonNode rootNode, String pathWithAttributeExpression) throws JRException {
        ...
        final int indexOfLeftRoundBracket = pathWithAttributeExpression.indexOf(ATTRIBUTE_LEFT);
        if (indexOfLeftRoundBracket != -1) {
            ...
            if(rootNode != null && !rootNode.isMissingNode()) {
                ...
                final String path = pathWithAttributeExpression.substring(0, indexOfLeftRoundBracket);

                // an expression in a form like: attribute==value
                final String attributeExpression = pathWithAttributeExpression.substring(indexOfLeftRoundBracket + 1, pathWithAttributeExpression.length() - 1);
                ...
                if (rootNode.isObject()) {
                    ...
                    if (!rootNode.path(path).isMissingNode()) {
                        if (rootNode.path(path).isObject()) {
                            ...
                            for (final JsonNode node: rootNode.path(path)) {
                    ...
                    for (final JsonNode node: rootNode) {
                        final JsonNode deeperNode = node.path(path);
                        if (!deeperNode.isMissingNode()) {
                            if (deeperNode.isArray()) {
                                for(final JsonNode arrayNode: deeperNode) {
    ...
    protected JsonNode goDownPath(JsonNode rootNode, String simplePath) {
        if(rootNode != null && !rootNode.isMissingNode()) {
            ...
            if (rootNode.isObject()) {
                ...
                result = mapper.createArrayNode();
                for (final JsonNode node: rootNode) {
                    final JsonNode deeperNode = node.path(simplePath);
                    if (!deeperNode.isMissingNode()) {
                        if (deeperNode.isArray()) {
                            for(final JsonNode arrayNode: deeperNode) {
                                ((ArrayNode)result).add(arrayNode);
    ...
    public JsonDataSource subDataSource(String selectExpression) throws JRException {
        ...
        final JsonDataSource subDataSource = new JsonDataSource(currentJsonNode, selectExpression);
