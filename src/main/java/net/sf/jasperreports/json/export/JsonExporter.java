...
public class JsonExporter extends JRAbstractExporter<JsonReportConfiguration, JsonExporterConfiguration, JsonExporterOutput, JsonExporterContext>
{
    ...
    private final List<HyperlinkData> hyperlinksData;
    ...
    {
        final List<PrintBookmark> bookmarks = jasperPrint.getBookmarks();
    ...
    protected void exportReportConfig() throws IOException
    {
        ...
        final List<JRPropertiesUtil.PropertySuffix> viewerProps = propertiesUtil.getAllProperties(
                jasperPrint,
    ...
    {
        final PrintParts parts = PartsUtil.instance(jasperReportsContext).getVisibleParts(jasperPrint);
    ...
    public static void writeParts(JasperPrint jasperPrint, PrintParts parts, Writer writer) throws IOException
    {
        ...
        final Iterator<Map.Entry<Integer, PrintPart>> it = parts.partsIterator();

        while (it.hasNext())
        {
            final Map.Entry<Integer, PrintPart> partsEntry = it.next();
            final int idx = partsEntry.getKey();
    ...
    {
        final HtmlResourceHandler fontHandler = getExporterOutput().getFontHandler();
        final ReportContext reportContext = getReportContext();
        ...
        {
            final Map<String, HtmlFontFamily> fontsToProcess =
                (Map<String, HtmlFontFamily>)reportContext.getParameterValue(REPORT_CONTEXT_PARAMETER_WEB_FONTS);

            final ObjectMapper mapper = new ObjectMapper();
            final ArrayNode webFonts = mapper.createArrayNode();

            for (final HtmlFontFamily htmlFontFamily : fontsToProcess.values())
            {
                final ObjectNode objNode = mapper.createObjectNode();
                objNode.put("id", htmlFontFamily.getId());
                final String cssResourceName = HtmlFontUtil.getFontCSSResourceName(htmlFontFamily);
    ...
    {
        final ReportContext reportContext = getReportContext();
        final String hyperlinksParameter = "net.sf.jasperreports.html.hyperlinks";
        if (reportContext != null && reportContext.containsParameter(hyperlinksParameter)) {
            final List<HyperlinkData> contextHyperlinksData = (List<HyperlinkData>) reportContext.getParameterValue(hyperlinksParameter);
            hyperlinksData.addAll(contextHyperlinksData);
        }
        if (hyperlinksData.size() > 0) {
            final String id = "hyperlinks_" + (hyperlinksData.hashCode() & 0x7FFFFFFF);
            if (gotFirstJsonFragment)
            {
                writer.write(",\n");
            } else
    ...
    {
        final ReportContext reportContext = getReportContext();
        final String clickableElementsParameter = "net.sf.jasperreports.html.clickable.elements";
        if (reportContext != null && reportContext.containsParameter(clickableElementsParameter))
        {
            final Boolean useClickableElements = (Boolean) reportContext.getParameterValue(clickableElementsParameter);
            if (useClickableElements)
            {
                final String id = "clickable_" + UUID.randomUUID();
                if (gotFirstJsonFragment)
                {
                    writer.write(",\n");
                } else
    ...
    {
        final GenericElementJsonHandler handler = (GenericElementJsonHandler)
                GenericElementHandlerEnviroment.getInstance(getJasperReportsContext()).getElementHandler(
                        element.getGenericType(), JSON_EXPORTER_KEY);

        if (handler != null)
        {
            final String fragment = handler.getJsonFragment(exporterContext, element);
    ...
    protected String resolveHyperlinkURL(int reportIndex, JRPrintHyperlink link)
    {
        ...
        if (!ignoreHyperlink)
        {
            final JRHyperlinkProducer customHandler = getHyperlinkProducer(link);
